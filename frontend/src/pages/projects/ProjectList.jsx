...
const ProjectList = () => {
  ...
  const debouncedSearch = useCallback(
    debounce(async (term) => {
      if (term.trim()) {
        ...
        if (result.data) {
          setDisplayedProjects(result.data);
        }
      } else {
        clearSearch();
      }
  ...
  // Update displayed projects when projects change or when sorting/pagination changes
  useEffect(() => {
          ...
          {displayedProjects.length > 0 && (
            <div className="flex flex-col sm:flex-row justify-between items-center mt-6 gap-4">
              <div className="text-sm text-gray-700">
                Showing {indexOfFirstItem + 1} to {Math.min(indexOfLastItem, displayedProjects.length)} of {displayedProjects.length} entries
              </div>
