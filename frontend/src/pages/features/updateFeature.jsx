...
const UpdateFeature = () => {
  ...
  const { id } = useParams();
  const { updateFeature, getSingleFeature, loading, error, clearError } = useFeatureContext();
  ...
  // Fetch the feature data when component mounts
  useEffect(() => {
    const fetchFeature = async () => {
      setFetchLoading(true);
      const result = await getSingleFeature(id);
      ...
      if (result.data) {
        ...
        // Handle case when feature is not found
     ...
     <div className='mx-auto max-w-xl text-center bg-gray-50'>
            <div className="bg-white p-12 rounded-2xl shadow-2xl transition-all duration-300 hover:translate-y-[-3px]">
              <div className="flex flex-col items-center justify-center gap-3 mb-6">
                <h1 className='text-2xl font-bold text-gray-900'>Update Feature or Report Bug</h1>
              </div>
