...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    if (selectedCategory !== 'All') {
      filtered = filtered.filter(task => {
        ...
        return project && project.category?._id === selectedCategory;
      });
    }
  ...
  const categoryData = categories.map(category => {
    const count = filteredTasks.filter(task => {
      const project = projects.find(p => p._id === task.project?._id);
      return project && project.category?._id === category._id;
    }).length;
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredTasks.length === 0 ? (
                  ...
                  filteredTasks.map(task => {
                    const project = projects.find(p => p._id === task.project?._id);
                    const category = categories.find(c => c._id === project?.category);
                      ...
                      <tr key={task._id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-purple-100 text-purple-800">
                            {task.project.category?.title || 'No Category'}
                          </span>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
